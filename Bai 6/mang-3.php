<?php
    $mang_so = array();
    $mang_duy_nhat = array();
    $so_lan_xuathien = array();
    
    if(isset($_POST['nhap_mang'])){
	$mang_so = explode(",", $_POST['nhap_mang']);
	$mang_duy_nhat = array_unique($mang_so);
	$so_lan_xuathien = array_count_values($mang_so);
    }
    $chuoi = "";
	foreach($so_lan_xuathien as $key => $value){
...
<body>
	...
	<table border="1">
		...
		<tr>
		    ...
		    <td><input type="text" name="so_lan_xuat_hien" value="<?php echo $chuoi; ?>"></td>
	        </tr>
	        <tr>
		    <td>Mảng duy nhất:</td>
		    <td><input type="text" name="mang_duy_nhat" value="<?php mang_duy_nhat($mang_duy_nhat); ?>" ></td>
	        </tr>
	        <tr>
		    <td colspan="2">
